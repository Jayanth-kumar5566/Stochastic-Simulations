...
tr_val=numpy.linspace(1e-8,1e-1,20)
...
for (n,m) in zip(tr_val,tr_val):
    ...
    y=[]
    z=[]
    for i in numpy.arange(0.0002,0.0010,0.00001):
        [(ser_t1,tim_t1),(ser_t2,tim_t2),(ser_tot,tim_tot)]=sim_av(0.0002,i,n,m)
    ...
    def sl(y,x=Xax):
        a=polyfit(x,y,1)
        return a[0]
    da=[sl(mi),sl(me),sl(ma),sl(z)]
    data.append(da)
    variance.append(numpy.var(da))
    ...
    plt.legend(loc='best')
    plt.savefig(str(count)+'.eps', format='eps', dpi=1200)
    count +=1
plt.clf()
plt.plot(tr_val,variance,'o')
plt.savefig("varianceplot.png",dpi=1200)
